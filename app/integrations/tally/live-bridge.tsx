...
export default function LiveBridge({onReview}:{onReview:(xml:string,name:string)=>void}){
 ...
 {recon&&<div>
  ...
  {!!recon.unexplainedCount&&<div><p className="tally-note"><b>{recon.unexplainedCount} income or expense ledger{recon.unexplainedCount===1?"":"s"}</b> ({drcr(recon.unexplainedTotal||0).replace(/ (Dr|Cr)$/,"")} in total) hold balances Commons has no vouchers for. That usually means vouchers are still held back or not yet imported.</p><ul>{recon.unexplained?.map(r=><li key={r.name}>{r.name}: {drcr(r.amount)}</li>)}</ul></div>}
