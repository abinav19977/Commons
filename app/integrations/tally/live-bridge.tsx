...
type Recon={booksFrom:string|null;hasBalances:number;openingsPosted:boolean;openingLedgers:number;checked:number;matched:number;differing:number;totalAbsDifference:number;unexplained?:{name:string;amount:number}[];unexplainedCount?:number;unexplainedTotal?:number;rows:{name:string;group:string|null;tally:number;commons:number;diff:number}[];vouchers:number};
...
export default function LiveBridge({onReview}:{onReview:(xml:string,name:string)=>void}){
 ...
 <div className="bridge-review"><h3>Match with Tally</h3><p className="tally-note">Reads Tally's Trial Balance and works out each ledger's opening balance as Tally's closing minus the vouchers Commons has imported. Posting that makes Commons agree with Tally, and it shows any income or expense that Commons cannot explain.</p>
 ...
 {recon&&<div>
  ...
  {!!recon.unexplainedCount&&<div><p className="tally-note"><b>{recon.unexplainedCount} income or expense ledger{recon.unexplainedCount===1?"":"s"}</b> ({drcr(recon.unexplainedTotal||0).replace(/ (Dr|Cr)$/,"")} in total) start the books with a balance that did not come from vouchers. That is normal when Tally has an opening balance entered directly on an income or expense ledger; otherwise check for held-back vouchers.</p><ul>{recon.unexplained?.map(r=><li key={r.name}>{r.name}: {drcr(r.amount)}</li>)}</ul></div>}
