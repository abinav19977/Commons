...
import { AlertTriangle, Check, Download, FileCheck2, RefreshCw, Sparkles, Upload, X } from "lucide-react";
...
type MappingSuggestion={name:string;accountCode:string;confidence:number;reason:string};
...
export default function TallyWorkspace({ counts, defaultFrom, defaultTo }: { counts: { customers:number;suppliers:number;products:number;vouchers:number;imported:number }; defaultFrom:string;defaultTo:string }) {
  ...
  async function chooseFile(event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0]; setPreview(null); setMessage(""); setError("");setXml("");setFileName("");setConfirming(false);setMappings({});setSuggestions({});
    if (!file) return;
    try {
  ...
  async function suggestMappings(){
    if(!preview||!unknownLedgers.length||busy)return;setBusy(true);setError("");setMessage("Asking AI for conservative mapping suggestions…");
    try{const saved=sessionStorage.getItem("commons.llm.session");const key=saved?(JSON.parse(saved) as {apiKey?:string}).apiKey:"";if(!key)throw Error("Connect and verify an OpenAI API key in Settings first.");
      const ledgers=unknownLedgers.map(name=>({name,voucherTypes:[...new Set(preview.vouchers.filter(v=>v.issues.some(issue=>issue.includes(`Map ledger “${name}”`))).map(v=>v.type))]}));
      const response=await companyFetch("/api/integrations/tally/suggest-mappings",{method:"POST",headers:{"content-type":"application/json","x-commons-ai-key":key},body:JSON.stringify({ledgers})});const body=await response.json() as {suggestions?:MappingSuggestion[];message?:string};if(!response.ok)throw Error(body.message||"AI suggestions could not be prepared.");
      const next=Object.fromEntries((body.suggestions||[]).map(item=>[item.name.toLowerCase().trim(),item]));setSuggestions(next);setMappings(current=>({...current,...Object.fromEntries(Object.entries(next).filter(([,item])=>item.confidence>=85&&item.accountCode).map(([name,item])=>[name,item.accountCode]))}));setMessage("High-confidence suggestions are selected. Review every row, then apply and recheck.");
    }catch(e){setError(e instanceof Error?e.message:"AI suggestions could not be prepared.");setMessage("");}finally{setBusy(false);}
  }
  return <>
