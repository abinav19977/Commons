...
export async function POST(request:Request){
 ...
 if(body.action==="masters"){
  ...
  for(const l of ledgers){
   ...
   if(!Object.hasOwn(l,"topGroup")){statements.pop();statements.push(raw.prepare("INSERT INTO tally_masters(id,owner_user_id,kind,name,top_group,opening_paise,closing_paise,updated_at) VALUES (?,?,'ledger',?,NULL,?,?,?) ON CONFLICT(owner_user_id,kind,name) DO UPDATE SET opening_paise=COALESCE(excluded.opening_paise,tally_masters.opening_paise),closing_paise=COALESCE(excluded.closing_paise,tally_masters.closing_paise),updated_at=excluded.updated_at").bind(crypto.randomUUID(),bridge.owner_user_id,l.name.trim().slice(0,200),money(l.openingPaise),money(l.closingPaise),now));}
  }
